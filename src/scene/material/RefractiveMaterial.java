	...
	public Color3f getColor(Scene scene, Hit hit, int recursionDepth) {
		...
		if (hit.getRay().getDirection().dotProduct(hit.getNormal()) <= 0) {
			...
			if (nextRay != null) {
				...
				Color3f colorUnattenuated;

				if (nextHit == null) {
					colorUnattenuated = scene.getBackground(hit.getRay().getDirection());
				} else {
					colorUnattenuated = nextHit.getSurface().getMaterial().getColor(scene, nextHit, recursionDepth + 1);
				}
		...
		nextColor = new Color3f(kr * nextColor.getRed(), kg * nextColor.getGreen(), kb * nextColor.getBlue());
